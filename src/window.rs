...
impl Tab {
    ...
    pub fn edit(figure: Figure) -> Rc<RefCell<Tab>> {
        Rc::new(
            RefCell::new(
                Tab::Edit {
                    btn_pressed: false,
...
impl Window {
    pub fn update(&mut self, handle: &RaylibHandle) {
        ...
        match &mut *tab {
            Tab::Edit { figure, btn_pressed, .. } => {
                figure.update(handle, btn_pressed);
            },
            _ => ()
        }
    }
    ...
    pub fn draw(&self, handle: &mut RaylibDrawHandle) {
        let tab = (*self.selected_tab).borrow();
        let width = 80;
        ...
        match &*tab {
            Tab::Edit { 
                ...
                let mut icons = vec![];
                icons.push(CString::new(handle.gui_icon_text(VetorIcons::ICON_CIRCLE.into(), None)).unwrap());
                icons.push(CString::new(handle.gui_icon_text(VetorIcons::ICON_LINE.into(), None)).unwrap());
                icons.push(CString::new(handle.gui_icon_text(VetorIcons::ICON_HEXAGON.into(), None)).unwrap());
                icons.push(CString::new(handle.gui_icon_text(VetorIcons::ICON_COPY.into(), None)).unwrap());
                icons.push(CString::new(handle.gui_icon_text(VetorIcons::ICON_CIRCLE_LINED.into(), None)).unwrap());
                icons.push(CString::new(handle.gui_icon_text(VetorIcons::ICON_CROSS_BOLD.into(), None)).unwrap());
                icons.push(CString::new(handle.gui_icon_text(VetorIcons::ICON_DIVIDE.into(), None)).unwrap());
                icons.push(CString::new(handle.gui_icon_text(VetorIcons::ICON_UNDEFINED.into(), None)).unwrap());
                icons.push(CString::new(handle.gui_icon_text(VetorIcons::ICON_ROOT.into(), None)).unwrap());
                icons.push(CString::new(handle.gui_icon_text(VetorIcons::ICON_VERTEX_FORMAT.into(), None)).unwrap());
                ...
                for (i, icon) in icons.iter().enumerate() {
                    ...
                    handle.gui_button(rrect(start.x + factorx, start.y + factory, icon_width, icon_width), Some(&icon));
                }
        ...
        for (i, tab) in self.tabs.iter().enumerate() {
            ...
            let text = match &*tab.borrow() {
                Tab::Edit { btn_pressed, circle, clone, delete, change_root, figure } => "Editar",
                Tab::Animation { btn_pressed, play, repeat, add_frame } => "Animar"
            };
